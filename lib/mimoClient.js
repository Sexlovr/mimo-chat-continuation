 ...
 */
export async function fetchMimoStream(account, query, modelConfig, conversationId, msgId, abortSignal, isEditedQuery = false) {
  const phEncoded = encodeURIComponent(account.ph_token);
  const url = `${BASE_URL}?xiaomichatbot_ph=${phEncoded}`;

  const cookie = `xiaomichatbot_serviceToken="${account.service_token}"; userId=${account.user_id}; xiaomichatbot_ph="${account.ph_token}"`;

  const body = {
    msgId: msgId,
    conversationId,
    query,
    isEditedQuery: isEditedQuery,
    modelConfig,
    multiMedias: []
  };
