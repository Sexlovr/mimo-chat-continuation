...
export function initDB() {
  ...
  db.exec(
    ...
    // Marker-based session tracking (replaces fingerprint approach for pro models)
    ...
    '  last_used DATETIME DEFAULT CURRENT_TIMESTAMP,' +
    '  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,' +
    '  UNIQUE(marker_id, api_key_hash)' +
    ');' +

    'CREATE INDEX IF NOT EXISTS idx_msg_marker ON messages(marker_id, api_key_hash);' +
    'CREATE INDEX IF NOT EXISTS idx_msg_last_used ON messages(last_used);'
  );
  ...
  try {
    ...
    var hasRootMsgCount = tableInfo.some(col => col.name === 'root_message_count');
    if (!hasRootMsgCount) {
      db.exec("ALTER TABLE conversations ADD COLUMN root_message_count INTEGER");
      console.log('[DB] Migrated conversations table to include root_message_count');
    }
  } catch (e) {
...
export function cleanupOldConversations() {
  ...
  var deletedMsgs = db.prepare(
    "DELETE FROM messages WHERE last_used < datetime('now', ?)"
  ).run(modifier);
  if (deletedMsgs.changes > 0) {
    console.log('[Cleanup] Removed ' + deletedMsgs.changes + ' stale marker rows');
  }
}
