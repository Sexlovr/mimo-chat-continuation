...
export function generateConvKey(messages, headerConvId) {
  ...
  // Include ALL non-system messages up to the 4th one (greeting + first response divergence)
  const nonSystem = messages.filter(m => m.role !== 'system');
  const earlyMessages = nonSystem.slice(0, 4).map(m => `${m.role}:${m.content || ''}`).join('|||');

  // Total message count prevents identical sliding window slices from colliding
  // The user specifically relies on this behavior to forcefully inject massive history trees into New Mimo Sessions on every turn.
